...
public class JXMapKit extends JPanel
{
	 ...
	 */
	public JXMapKit(boolean enableDefaultOverlay)
	{
		initComponents();
...
//				Point2D centerpos = map.getTileFactory().geoToPixel(mapCenterPosition, map.getZoom());
	...
	public void setTileFactory(TileFactory fact)
	{
		...
		miniMap.setZoom(Math.max(fact.getInfo().getDefaultZoomLevel() - 3, fact.getInfo().getMinimumZoomLevel()));
	...
	/*public static void main(String... args)
	{
		SwingUtilities.invokeLater(new Runnable()
		{
			...
			public void run()
			{
				JXMapKit kit = new JXMapKit(true);
